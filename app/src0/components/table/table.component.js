...
const Table = ({ tableKeys, tableContent = [], _rowFormatter = TableRow }) => {
    ...
    const bottomBound = (container, lastElement) => {
        var size = container.getBoundingClientRect().top + container.getBoundingClientRect().height - (lastElement.getBoundingClientRect().top + lastElement.getBoundingClientRect().height) + 1
        return (
...
const handleScrollTable = (e) => {
    ...
    if(displayedElementsIndexes.includes(tableContent.length-1) && direction == 'DOWN'){
      return(null)
    }
